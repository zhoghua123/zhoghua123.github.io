...
![图1](https://raw.githubusercontent.com/zhoghua123/imgsBed/master/wlxy-08.png)

1. 场景：计算机2（下面用A代替）向计算机3（下面用B代替）发送数据.网关1（192.168.3.1），网关2（192.168.4.1）
2. A先发送ARP广播获取网关1的MA的地址
3. A通过ICMP协议发送数据给网关1
4. 路由器拿到网关1的数据携带的目的IP地址，扎到网关2
5. 网关2首先发送ARP协议给B，获取B的MAC地址
6. 网关2通过ICMP协议发送数据给B
7. A向B发送一条跨网数据流程完成
