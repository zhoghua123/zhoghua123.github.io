...
3. **flex container与flex items的概念**
    1. 开启了flex布局的元素叫 **flex container**：一旦一个元素使用了flex布局就叫它flex容器（flex container）
...
2. flex-direction 决定了main axis 的方向，有4个取值：row（默认值）、row-reverse、column、column-reverse
