...
1. stat函数
    ...
    3. 使用：
        ...
        2. 运行： `./stat english.txt`
        3. 打印： `file size = 95`
2. access函数
    1. 作用：测试文件有哪些权限
3. 其他函数：`chmode、chown、truncate、link、symlink、readlink、unlink、rename`
...
2. `man dup`
    ...
    ```
    #include <unistd.h>
    int dup(int oldfd); //返回的事文件描述符表中没有被占用的最小的文件描述符
    int dup2(int oldfd, int newfd); 
    //old->new 如果new是一个被打开的文件描述符，在拷贝前先关掉new
    //old 和new是同一个文件描述符，不会关掉new，直接返回old
    ```
#### fcntl函数
1. 作用： 改变已经打开的文件的属性
