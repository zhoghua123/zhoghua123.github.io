...
1. 案例：
    ...
    1. 可以看见Person()、 Person(int, int)都调用了，那为什么没有初始化成功呢？（可以由反汇编分析）
        ...
        3. 构造函数内部调用构造函数Person(0,0);
        4. 调用构造函数Person(0,0)时，会将**this指针的地址**传递给Person(0,0)构造函数内部的this指针。
        5. 那么Person(0,0)构造函数内部的this指针指向的可不是对象的地址了，而是Person()内部this指针的地址值，**并不是Person()内部this指针指向的对象地址**
        6. 因此，Person(0,0)内部使用this赋值肯定失败
2. 结论:**构造函数调用构造函数必须在初始化列表中调用**
    ...
    1. 这么调用就会把Person()内部的this指针**指向的地址值（不是this的地址值）**传递给Person(int,int)
