...
2. 内存图(x86环境的图)
    ...
    ![图1](https://raw.githubusercontent.com/zhoghua123/imgsBed/master/cpp09.png)   
 ...
 3. 虚表汇编分析
    ...
    1. 调用speak
        ...
        002929F8  mov         ecx,dword ptr [cat]  
        //取出虚表中前面4个字节存储的数据（就是0x00B814E7）给eax
    ...
    2.  调用run
        ...
        //将cat对象的地址传递给虚函数内部的this指针
        00292A0D  mov         ecx,dword ptr [cat] 
       //取出虚表中后面4个字节存储的数据（就是0x00B814CE）给eax
