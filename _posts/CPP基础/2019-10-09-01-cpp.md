...
2. 视图类：
    1. 工具栏->视图->类视图
    2. 可以看到当前项目中生成的5个类（CAboutDlg、CMainFrame、CMFCGuideApp、CMFCGuideDoc、CMFCGuideView）
    3. 双击类名即可以进入该类的.h文件，分栏下部分显示该类的成员函数
    4. 双击成员函数，既可以跳转到改类cpp文件中的实现
3. 文档/视图结构体系
    ...
    3. CView类也派生于CWnd类，框架窗口是视图窗口的一个父窗口。主框架窗口（CFrameWnd）是整个应用程序外框所包括的部分，即图中粗框以内的内容，而视类窗口只是主框架中空白的地方。
        ![图4](https://raw.githubusercontent.com/zhoghua123/imgsBed/master/cpp70.png)
4. 消息处理的添加
