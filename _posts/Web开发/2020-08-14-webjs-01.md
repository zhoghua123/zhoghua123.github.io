...
### 定时器
1. 两种定时器，window 对象给我们提供了 2 个非常好用的方法-定时器。`setTimeout()`、`setInterval()`
...
2. 停止 setInterval() 定时器
    ...
    2. 注意：window 可以省略。，里面的参数就是定时器的标识符 。
3. 代码举例：
...
### location 对象
1. 什么是 location 对象：window 对象给我们提供了一个 **location 属性**用于**获取或设置窗体的 URL**，并且可以用于**解析URL**。 因为 这个属性返回的是一个对象，所以我们将这个属性也称为**location 对象**。
2. URL
