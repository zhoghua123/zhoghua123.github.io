...
1. 什么是JavaScript?
    ...
    2. 解释型语言：
        ...
        3. 为什么JavaScript不需要编译就能够执行呢？ 与浏览器的工作原理有关，因为浏览器有个`JavaScript Interpreter(解析者) `JavaScript解析器，专门负责解析、执行JavaScript的代码
    3. 编译型语言：程序执行之前，需要一个专门的编译过程，把程序编译成为机器语言的文件，比如exe文件
2. 浏览器的工作原理：浏览器由下面7个部分组成如下图：
...
1. 在JS中，打印日志输出，通常有以下几种方法：
    1. 在控制台中显示内容：在浏览器的F12下，点击console可以查看日志
...
5. 特殊字符常量
    ...
    \" 输出"字符,也就是在屏幕上显示一个"字符。
    ```
6. 自定义常量：ES6中新增，自定义常量，`const 常量名称 = 常量取值;`
...
1. 关键字
    ...
    2. 关键字严格区分大小写, var和Var前者是关键字, 后者不是
        ...
        ```  
2. 保留字：JavaScript预留的关键字，他们虽然现在没有作为关键字，但在以后的升级版本中有可能作为关键字
...
1. 静态数据与动态数据
    ...
    5. 数据的计量单位：不管是静态还是动态数据，都是0和1组成的，数据越大，包含的0和1就越多
2. JavaScript数据类型概述
...
#### Null和Undefined
1. Undefined这是一种比较特殊的类型，表示**变量未赋值**，这种类型只有一种值就是undefined
