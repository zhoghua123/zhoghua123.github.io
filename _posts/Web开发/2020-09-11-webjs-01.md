...
1. Express框架中使用req.query即可获取GET参数，框架内部会将GET参数转换为对象并返回。
    ...
    ```
2. Express中接收post请求参数需要借助第三方包 body-parser。
...
3. **注意：views文件夹中.art模板文件引用的静态资源路径必须是绝对路径！！！**
    ...
    1. 如果访问login.art的请求为：`http://localhost/abc/login`
    2. 那么如果该模板页面引用的为相对路径，那么该访问css的路径为：`http://localhost/abc/lib/bootstrap/css/bootstrap.min.css`，然而项目的public文件夹下面并没有abc以及后面的路径,相对的根目录为：`http://localhost/abc/`
    3. 如果为绝对路径，则css访问路径为：`http://localhost/admin/lib/bootstrap/css/bootstrap.min.css`，绝对的根目录为`http://localhost`
