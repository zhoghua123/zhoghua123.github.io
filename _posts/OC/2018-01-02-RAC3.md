...
11. 底层实现
    1. `bind:`方法内部简析:
        ...
        return [[RACSignal createSignal:^(id<RACSubscriber> subscriber) {
            //调用外部传进的block,返回RACSignalBindBlock
    ...
    2. 结论
        ...
        2. 当绑定信号(`bindsignal`)被订阅,就会调用绑定信号中的`didSubscribe`
            1. 调用`bind:`传入的`block`,返回一个`bindingBlock(RACSignalBindBlock)`。
            2. 内部订阅外部源信号,`nextBlock`
        3. 当源信号有内容发出(发送信号)，调用bind内部的订阅`nextBlock`,不会调用外部的`nextBlock`,因为`bindSignal`此时还为nil呢,还没有返回呢
        4. `bind`内部调用`bindingBlock`,外部`block`实现中进行处理,并返回处理后的`signal`,此时`bindSignal`有值了
        5. 调用`bindSignal`的`nextblock` 
