...
4. 手动控制一个线程的声明周期
    ...
    3. 代码分析结果：
        ...
        3. 这么创建，会发现控制器delloc了，但是self.thread仍然没有销毁
            1. 因为self.thread内部用runloop，一直处于休眠状态，那么self.thread久必须一直处理事情，所以self.thread不死
            2. 那么接下来需要退出runloop
        4. 实现手动退出runloop
            1. 添加一个退出按钮事件
            ...
            3. 那么将这个方法替换成下面的方法
                ...
                1. 换成这个方法后，点击touch界面，处理一次子线程事件后发现，runloop直接退出了
                2. 这个方法，每次掉用就会启动一次runloop，当前线程只要执行一次事件就会自动退出当前的runloop
