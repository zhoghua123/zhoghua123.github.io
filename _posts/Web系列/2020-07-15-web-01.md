...
1. 盒子模型（Box Model）
    1. HTML中的每一个元素都可以看做是一个盒子，如下图所示，可以具备这4个属性
        ...
        1. 内容（content）： 盒子里面装的东西
        2. 内边距（padding）：怕盒子里装的东西损坏而添加的泡沫或者其他抗震的辅料
        3. 边框（border）： 就是盒子的边框，比如木盒子四周的木板
        4. 外边距（margin）：为了方便取出，盒子之间保留一定的空隙
    2. 默认的盒子模型如下图所示
...
### margin注意点
#### 上下margin传递
1. margin-top传递：如果**块级**元素的**顶部线**和**块级**父元素的**顶部线**重叠，那么这个**块级**元素的margin-top值会传递给父元素，代码举例：
...
2. margin-bottom传递：如果**块级**元素的**底部线**和**块级**父元素的**底部线**重叠，并且父元素的高度是auto，那么这个**块级**元素的margin-bottom值会传递给父元素，代码举例：
...
3. 折叠后最终的计算规则
    1. 如果都是**正**数，最终值是：绝对值**最大**的那个**正**数值
...
3. 应用实例:去除a元素，input元素的focus轮廓效果
4. 举例使用：
    1. 例1：
...
1. box-sizing用来设置盒子模型中宽高的行为
    1. content-box: padding、border都布置在width、height**外面**
    2. border-box: padding、border都布置在width、height**里面**
2. 图文解释
    1. content-box
...
2. 子元素是:行内级元素、inline-block元素
    ...
    2. 可以这么理解为什么text-align针对bolck元素无效
        ...
        3. 但是block尽管也可以设置width，表面看起来与父元素之间有了空隙，但是由于block的特性是占据父元素的整行，即使宽度设置有剩余也不允许其他元素占据，因此实际上还是占据了父元素整行，所以无法居中
3. 子元素是：块级元素
