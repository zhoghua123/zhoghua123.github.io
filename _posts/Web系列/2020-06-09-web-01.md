...
2. 缓存技术
    ...
    3. 好处： 减少网络请求次数、节省用户流量、加快网页响应速度、提升用户体验
5. 网页从无到显示的过程：
    1. 前端工程师开发网站项目
    2. 将网站项目上传部署到服务器
...
3. 建议： 
    1. 前端工程师需要特别关注的浏览器有： Google Chrome、Mozilia Firefox（火狐）、Opera（欧朋）、Safari、IE
    2. 建议使用Google Chrome浏览器为默认浏览器，功能强大、快速稳定
4. **查找一个HTML元素、CSS属性支撑哪些浏览器以及该浏览器的版本：**  [Can I use 查询](https://www.caniuse.com/)
  
### 浏览器调试技巧总结（持续补充）
1. 如何快速进入代码检查：在当前网页点击F12，进入代码检查
...
## HTML简介
1. HTML 文件：HTML文件的扩展名是.htm/.html，因历史遗留问题，Win95、Win98系统的文件拓展名不能超过3字符，所以使用.htm，一般我们也可以称html文件为网页文件
2. 什么是HTML
...
6. 元素嵌套元素
    1. 一个元素里面可以嵌套其他元素
        ...
        ```
    2. div是span的父元素，span是div的子元素，br与span是兄弟元素，同一个父元素的元素称为兄弟元素
    3. 元素嵌套的意义是什么？
...
7. 元素的属性
    ...
    5. 有些属性是公共的，每一个元素都可以设置， 比如：class、id、title、lang属性
    6. 有些属性是元素特有的，不是每一个元素都可以设置，比如mata元素的charset属性
8. 注释：用来解释某段代码的具体含义，格式：`<!-- 注释内容-->`
9. HTML的发展史
    ...
    6. **2014 HTML 5.0:** HTML5 是一个全新的HTML标准，优势极其明显
        ...
        4. 兼容旧版本，也废弃一些旧特性
10. HTML5 官方文档
    1. 官方文档才是最权威的资料[https://www.w3.org](https://www.w3.org)
    2. 国内教程
...
### h、p、strong元素
1. h元素：表示网页的标题，h1~h6共规定了6个等级的标题
2. p元素：表示文章中的一个段落（paragraph）
3. strong元素：用于强调某些文本，粗体的显示效果
4. 举例使用：
