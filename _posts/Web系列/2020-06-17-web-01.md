...
3. alt属性设置图片站位文字
4. **注意：**
    1. img元素如果只设置了width（或height），浏览器会自动根据图片宽高比计算出height（或者width）
...
### iframe元素
1. 利用iframe元素可以实现：在一个HTML文档中嵌入其他的HTML文档，即： 在当前网页中拿出来一块用于显示其他网页的内容
2. 常用属性：
    1. src：要显示的网页内容
...
3. URL常见的协议：
    ...
    3. mailto：访问的是电子邮件的地址，格式是：mailto:
...
4. 更具体的URL
    ...
    3. query：请求参数，提交给服务器的数据
    4. `#fragment`：**锚点的位置**

### 标签语义化
1. 什么是标签语义化？选择标签的时候要尽量让每一个标签都有正确的语义
