...
1. background-image用于设置元素的背景图片
    ...
    2. 在图片的透明区域，可以看到背景色
    3. 背景图片默认是平铺
2. 如果设置了多张图片，设置的第一张图片将显示在最上面，其图片按顺序层叠在下面
...
4. 如果设置了1个方向，另一个方向默认是center
    ...
    2. 比如`background-position:center`，等价于:`background-position:center center`
...
#### 背景大图片适配
1. 场景：有的网页上面整个背景就是个大图片，那么这个大图片是如何适配各种不同屏幕的尺寸呢？
2. 分析：
...
2. 总结：
    ...
    3. 宗旨：在没有任何CSS样式的情况下，用户也能浏览到网页中完整的内容信息（ps:网络出现问题或服务器出现问题时，有可能会导致CSS文件加载失败）
...
2. 伪元素`::first-line`的背景
    1. `::first-line`虽然意为第一行元素内容，但是他的背景并**不一定填满一行**，取决于各种因素，取决于：文字大小、容器宽度、文字对齐方式
...
3. cursor除了可以设置系统自带的一些值外，还可以设置图片
    ...
    3. 如果图片找不到，就会使用pointer作为cursor的值
