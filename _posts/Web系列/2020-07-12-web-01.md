...
3. w3C官方文档对img标签的说明：
    ...
    img是一个替换元素，它有个默认display值是inline，但是它的默认维度是由嵌入图像的内在值定义的，就像它是inline-block一样。你可以设置属性像：border/border-radius, padding/margin, width, height, etc. on an image.
    ```
...
#### display的其他属性值
1. display的一下取值，等同于某些HTML元素 
    
    ```
    1. table： <table>,一个block-level表格
    2. inline-table：一个line-level表格
    3. table-row： <tr>
...
1. overflow用于控制内容溢出时的行为
    ...
    2. hidden：溢出的内容直接裁剪
    3. scroll:溢出的内容呗裁剪，但是可以通过滚动机制查看
