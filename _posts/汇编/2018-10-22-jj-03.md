...
2. 总结
    ...
    3. 部分代码如下：（下面这段代码，即使是监听C语言的反汇编，也是同样的代码）
        ...
        1. 图片分析
            ![pic-w50](https://raw.githubusercontent.com/zhoghua123/imgsBed/master/hbjj17.png)
...
2. 代码举例：
    ...
    ```
3. 内存分析图如下:
     ![pic-w50](https://raw.githubusercontent.com/zhoghua123/imgsBed/master/hbjj18.png)
...
#### 保护可能会用到的寄存器 
1. 从上面分析的保护BP我们可以看出，有些寄存器，一旦在函数内部修改了他，当函数调用完毕后，我们还要恢复他，那么这些寄存器就是**需要保护的寄存器**。 那么那些寄存器通常需要保护呢？
...
4. 内存分析图：
    ...
    ![pic-w50](https://raw.githubusercontent.com/zhoghua123/imgsBed/master/hbjj19.png)
...
2. 通过在Windows中反汇编C语言代码，我们可以看到，Windwos填充的是：CCCC，这个其实就是`int 3`中断向量码。
3. 那么如何把局部变量空间这段内存都初始化为：CCCC呢？ 
    1. 用附加段寄存器`es:di`来操作局部变量内存这段空间
    2. 只要将es：di 指向 ss:bp-10 就可以了
4. 完整的代码如下：
...
1. 栈帧(Stack Frame Layout)
    ...
    5. 函数3先调用函数1，接着调用函数2
        ...
        ![pic-w50](https://raw.githubusercontent.com/zhoghua123/imgsBed/master/hbjj20.png)
    6. 进入函数之后，栈中的情况
        
        ![pic-w50](https://raw.githubusercontent.com/zhoghua123/imgsBed/master/hbjj21.png)
    
    7. 具体图
        
        ![pic-w50](https://raw.githubusercontent.com/zhoghua123/imgsBed/master/hbjj22.png)
