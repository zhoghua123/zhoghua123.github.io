...
1. location 匹配后面有“/”跟没有“/”区别
    ...
    2. 没有斜杠：location /abc
        ...
        ```
    3. **注意：** 如果用"/"匹配，**必须确保覆盖当前资源里面所有请求路径**：比如只监听`/abc/`,访问uri为：`http://www.xxx.com/abc/login.html`,如果该页面内部访问后台接口为`http://www.xxx.com/tvbControl/login`,那么该接口将无法访问。解决方案如下：
        ...
        ```
2. proxy_pass 后面的斜杠
    1. 关键规则：proxy_pass 后面的斜杠决定是否替换 location 匹配的路径。
    2. 有斜杠：会去掉 location 匹配的路径部分，将请求 URI 中 location 匹配的部分替换为 proxy_pass 的路径。
        
        ```
        location /api/ {
            proxy_pass http://www.xxx.com/abc/;  # 有斜杠
        }
        # 请求 /api/user → 转发到 /abc/user
        ```
    3. 无斜杠：保留完整的请求 URI不进行路径替换
        
        ```
        location /api {
            proxy_pass http://www.xxx.com;  # 有斜杠
        }
        # 请求 /api/user → 转发到 /api/user
        ```
3. 如果ng1只能配置转发ip，但是ng2只监听域名(`www.xxx.com`)，那么如何通过ng1的ip转发至ng2呢？
    1. 通过设置：`proxy_set_header Host www.xxx.com`
    ...
    location /abc {
        ...
        access_log logs/ng86acmp-access-$year-$month-$day.log main;
    }
    ```
