...
2. 内存如下：
 ...
 ![图1](https://raw.githubusercontent.com/zhoghua123/imgsBed/master/javazh-06.png) 
...
1. 3个常见的注解
    1. @Override：告诉编译器这是一个重写后的方法
        1. 如果不加没有影响
        2. 如果加了，当我们重写的方法名、方法签名写错了，编译器会报错
...
### 访问控制（Access Control）
1. Java 中有 4 个级别的访问权限，从高到低如下所示
    1. public：在**任何地方**都是可见的
        1. main函数与Person在**不同的包中**，然后在main函数中创建一个Person对象，该对象可以访问自己的成员
    2. protected：仅在自己的包中、自己的**子类中**可见
        1. main函数与Person必须在**相同的包中**，然后在main函数中创建一个Person对象，该对象可以访问自己的成员
        2. 如果Student子类，跟Person**不在同一个包中**，内部实现中可以访问Person的成员
    3. 无修饰符（package-private）：仅在自己的**包中**可见
        1. 如果Student子类，跟Person**必须在同一个包中**，内部实现中可以访问Person的成员
    4. private：仅在自己的**类中**可见
        1. 如果Student子类，内部实现中**不可以访问Person的成员**
        2. main函数与Person在**相同的包中**，然后在main函数中创建一个Person对象，该对象不可以访问自己的成员
2. 使用注意
    1. 上述 4 个访问权限都可以修饰**类的成员**，比如成员变量、方法、嵌套类（Nested Class）等
    2. 只有 public、无修饰符（package-private）可以修饰顶级类（Top-level Class）
...
2. toString 方法来源于基类 java.lang.Object，默认实现如下所示
    ...
    ```
3. Eclipse 中有一个可以自动生成 getter、setter、constructor、toString 等常用代码的快捷键
    1. Shift + Alt + S
