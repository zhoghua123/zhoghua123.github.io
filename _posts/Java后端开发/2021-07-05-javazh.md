...
#### 注入bean对象
1. 如果bean属性本身有@Autowired,那么直接new原来对象即可（**前提条件是IoC容器中必须有这个bean参数对象**）
...
3. 直接调用`@Bean`方法来注入
    ...
    1. 这里有个细节：`@Bean`方法被直接调用多次,也能保证是singleton(同一个对象)
        1. 因为`@Configuration`底层使用了CGLib动态代理，对`@Bean`方法进行了增强处理
    

#### FactoryBean
1. 创建一个DogFactoryBean类
...
2. 如果创建工厂的入口是注解，一般就用AnnotationConfigApplicationContext
    ...
    2. 可以通过`@Import、@ComponentScan`扫描其他注解信息
        1. ComponentScan：只能扫描到带有`@Component`修饰的类;`@Configuration`也可以，打开源码可以看到它本质也是`@Component`
    ...
    4. 注意：通过`@Import`导入创建的bean的id是全类名
