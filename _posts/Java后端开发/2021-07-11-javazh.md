...
## 简介
1. SpringMVC的正式名称是Spring Web MVC，是属于Spring框架的一部分，是基于Server API的Web框架
...
4. 特点：必须是JAVAEE项目
    1. 前面讲的mybatis、spring主要是用来操作dao、service层，不涉及到servlet，那么也就意味着Java项目、JavaEE项目都通用。那么SpringMVC是专门处理客户端发送过来以及响应给客户端的请求的，因此只能用于JavaEE项目
...
2. 方法二：默认情况下，请求参数会传递给**同名**的方法参数
    1. 也可以通过`@RequestParam`指定方法参数对应的请求参数名
        1. required属性为false: 请求参数是可选的，客户端可以不传
        2. required属性为true: 请求参数是必须的，客户端必须传，否则出现400错误
        3. 如果用`@RequestParam`修饰，不写required，则默认为false
        4. 如果没有用`@RequestParam`修饰，则默认必传
