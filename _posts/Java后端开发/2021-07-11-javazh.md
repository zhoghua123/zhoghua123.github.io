...
1. 获取请求参数
    ...
    2. 方法二：默认情况下，请求参数会传递给**同名**的方法参数
        ...
        //如果不加@RequestParam，则默认是required = true，如果不加则默认是false
...
2. 请求路径变量
    1. 如果请求路径中的内容是变化的，可以定义成(变量名)，然后通过使用`@PathVariable`去获取值
        ...
        ```
3. 利用反射获取参数名
    1. 正常情况下方法只编译参数类型，不会编译参数名称，但是也可以通过特殊方式将方面名称也编译到可执行文件中，这样其他模块调用该方法时就可以直接通过参数名称传递
...
4. 示例:
    1. 在web.xml中，设置servlet后，要配置servlet拦截哪些请求通过Controller进行处理
        ...
        ```
    2. 这里设置的是`/`，拦截出jsp以外的请求，那么也会拦截html的请求
