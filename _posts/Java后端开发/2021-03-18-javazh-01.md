...
3. **不能**用作泛型方法调用、泛型类型实例化、泛型类型定义的类型参数
...
3. 例2：解决上述问题
    ...
    //实际调用的时候会进行全局替换，代码编译的时候都是编译到代码段
...
2. ?一般定义在引用变量上,T一般作用于定义方法上或者类上
3. T会进行全局替换成实际类型，?不会全局替换类型
4. **?可以认为不属于泛型，跟泛型没有关系**
5. **泛型定义方法时需要在方法前声明泛型参数,通配符不需要，而且仅仅用来用作方法的参数类型。**
6. 使用范围不同：
