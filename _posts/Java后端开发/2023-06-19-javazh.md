...
### Docker Swarm
1. 用来管理多机多容器的工具
2. Docker Compose：帮助在 同一个节点 上部署多个容器。
3. Docker Swarm：多台机器上部署容器。开箱即用，快速部署容器。偏重容器部署
4. K8s：社区活跃度高，组件丰富。微服务化，偏重应用的部署
5. compose支持在 同一节点 上部署，swarm支持在多个节点上部署容器。这两者都是docker原生支持的docker集群部署方式，开箱即用，比较方便，偏重于容器的部署。