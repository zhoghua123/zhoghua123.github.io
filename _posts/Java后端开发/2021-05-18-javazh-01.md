...
6. 在login.jsp文件中，验证码图片标签img的src属性中设置请求
    ...
    ```
7. **疑问：**
    1. User的bean模型并没有验证码字段，用户点击登录的时候，服务器理论上是拿不到用户输入的验证码字段的，那么如何将验证码发送给服务器？
    2. 假设服务器拿到用户输入的验证码数据，验证码图片是通过第三方库Kaptcha生成的，点击登录时服务器如何去比对用户输入的图片验证码数据？除非Kaptcha的服务数据与登录接口的服务数据是共享的
