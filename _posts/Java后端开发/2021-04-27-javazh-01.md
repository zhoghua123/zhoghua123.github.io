...
#### 新建一个Web项目
1. 新建一个Maven项目
    1. file ->new->module...->Maven->next->项目命名(10_TestWeb)、路径(`~/Desktop/JavaStudy/Codes/JavaProject/10_TestWeb`)、groupid(com.zh)、artifactid(10_TestWeb)、version(1.0.0)->点击finish
    2. 当前项目**仅仅是一个纯java项目**，不是web项目
...
2. 创建web项目部分
    ...
    5. 此时项目目录main文件夹中就多了一个webapp文件夹
        ...
        ```
3. 创建Servlet
    1. 右击java文件夹->new->Create New Servlet（此时有这个选项了）
...
1. 打开pom.xml添加servlet依赖
    ...
    ```
2. 集成tomcat7-maven-plugin插件（**使用Maven内置的Tomcat**）
...
### IDEA部署Maven项目  
1. 上面讲的用maven插件+本地Tomcat部署项目过程麻烦，有没有一种办法用IDEA上面配置的Tomcat来部署呢？
2. 非Maven的web项目打包的结果
...
4. IDEA非常智能
    ...
    2. 尽管项目中的第三方依赖jar不在WEB-INF下面的lib文件夹中，仅仅在pom.xml中，在打包的时候仍然可以将相应的第三方jar包打包到target中
5. **Maven的作用**
