...
2. Download & Build from Release
    ...
    ```
    Click here to download the 4.9.2 source release. Also you could download a binary release from here.
    ```
3. 二进制安装
4. 源码安装
    1. 源码安装和二进制差不多,主要的操作步骤, 是通过源码去生成对应的二进制文件, 需要依赖的条件:
        1. JDK 1.8+
        2. Maven 3.2(大于3.2)
    2. 下载源码`rocketmq-all-4.9.2-source-release.zip`
    3. 安装Maven(命令行)
    4. 进入到源码目录，解压源码
    ...
    6. 找到编译后的文件目录
        ...
        ```
    7. 可以将这个可执行软件复制到相应目录下(Linux环境，mac不用)
        
        ```
        cp -R rocketmq-4.9.2/ /usr/local/
        ```
    8. 修改配置参数
