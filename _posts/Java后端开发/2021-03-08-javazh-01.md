...
5. 语言的跨平台原理
    1. C\C++的跨平台原理
        ...
        ![图1](https://raw.githubusercontent.com/zhoghua123/imgsBed/master/javazh-01.png) 
        
        1. 原理：使用平台相关的编译器生成**对应平台**的可执行文件
    2. Java的跨平台原理
        
        ![图1](https://raw.githubusercontent.com/zhoghua123/imgsBed/master/javazh-02.png) 
...
1. Java开发必备
    ...
    3. 下图是从Java官方获取的图
        ![图1](https://raw.githubusercontent.com/zhoghua123/imgsBed/master/javazh-03.png) 
        
        1. JDK中已经包含JRE,JRE中已经包含JVM
        2. 因此只需要安装JDK即可
2. JDK的下载
...
3. JDK的安装
    ...
    2. 如果是windows系统安装如下：(假设我们下载的版本未：Java SE 8U231,即jdk1.8.0_231.jdk)
        1. 双击安装包，然后下一步
        2. 进入下图列表
        ...
        3. 安装完成，打开文件夹目录中就可以发现有个jre文件夹，说明包含了jre
            ...
            ![图1](https://raw.githubusercontent.com/zhoghua123/imgsBed/master/javazh-04.png) 
4. IDE
...
2. 常见问题
    ...
    5. PATH配置
        ...
        3. 如果【Path】是不存在的，【新建】→【输入变量名为Path和变量值为JDK的bin目录】→【确定】
        4. 如果Eclipse可以成功打开，看到了启动界面，说明PATH配置成功
        5. **注意：**window7系统配置时，Path路径下不是一个一个的列表可以添加，而是整个是一个输入框，里面很多路径，当新增一个路径与上一个路径之间要用英文的分号（;）隔开
        6. 以后可能会经常在环境变量中用到**JDK目录**，可以考虑将**JDK目录**定义为JAVA_HOME，方便重复利用
            1. 就是重新新建一个路径，变量名为：JAVA_HOME，变量值为**JDK的安装目录**(`D:\Dev\Java\jdk1.8.0_231\`)
            2. 然后在Path的路径中添加这个自定义的路径JAVA_HOME，那么该如何添加呢？
                1. 在Path中就可以使用`%JAVA_HOME%`引用JDK目录,结果为：`%JAVA_HOME%\bin`
                2. 甚至可以直接在**文件浏览器（就是文件夹的搜索框）**中输入`%JAVA_HOME%` ,敲回车后会自动跳转到JDK目录
            
            ![图1](https://raw.githubusercontent.com/zhoghua123/imgsBed/master/javazh-05.png) 
...
9. 新建一个class
    ...
    2. 右击src文件夹new-class,输入类名`FirstClass`,勾选public static void main
        1. 会自动实现main函数，程序的入口
        2. 注意：Java特点是所有方法必须放到类中，现有类才有方法
...
2. project SDK选择jdk1.8,javaEE version选择最新版本即可，Additional Libraries and Frameworks不选择，项目模板选择 Web Application，点击next
3. 输入项目名称(01_HelloWorld)、路径D:\Dev\Java\Codes
4. Artifact(构件)：只能用拉丁字符或者_开头
5. 组（Gruops）:就是包名，公司的名称反写
6. 点击完成
7. 也需要配置访问路径(/hello)，用来区分当前项目是服务器软件Tomcat下的哪一个项目。
8. 项目目录
...
5. 接收客户端发送过来的登录请求参数
    ...
    5. 在LoginServlet类中编写接收客户端代码：
        ...
        * */
        ...
        ```
    6. **注意：**
        1. 第2、3步骤可以不用设置，但是如果不设置，无法查看tomact的源代码，比如查看HttpServlet的源代码
...
6. 删除、导入、复制已存在的模块
    ...
    2. 导入: File->Project Structure... ->点击左上角+ ->import Module->找到目录的Module，在文件夹中找到xxx.iml文件夹（05_crm.iml），然后点击打开即可
    3. 复制：值接在项目列表复制粘贴，重新命名文件夹名字，比如原来05_crm复制后修改为06_crm,注意也要将文件目录下的05_crm.iml也修改为06_crm，然后再导入项目
