...
2. 检查型异常（Checked Exception）
    ...
    2. 哪些异常是检查型异常？：除 Error、RuntimeException以及其子类，以外的异常
3. 非检查型异常（Unchecked Exception）
    1. 这类异常一般可以避免，编译器不会进行检查：如果开发者没有处理这类异常，编译器将不会报错
    2. 哪些异常是非检查型异常？：Error、RuntimeException以及其子类
4. 常见的检查型异常
...
5. 常见的非检查型异常
    ...
    2. RuntimeException
        ...
        Double d = (Double)obj;
        ```
6. **总结：** 检查就是编译器对否会自动检查，这个方法是否会抛出异常、以及抛出异常的种类是否需要开发者必须用try-cach或者throws来处理。检查型异常类型必须处理，非检查型类异常可以不处理
