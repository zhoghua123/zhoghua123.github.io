...
1. skill.xml代码
    ...
    ```
2. 与上面相比，不用在WHERE后面添加`1=1`了,之前添加目的是因为后序拼接的语句会多余一个AND
3. 如果用了where会自动删除多余的AND
...
## 集成druid连接池
1. 在最开始讲mybatis-config.xml的时候，将配置数据库
...
2. 但是通常我们不适用mybatis内置的连接池，通常使用其他封装的连接池，更好用，那么如何使用呢？
