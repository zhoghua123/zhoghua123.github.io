...
10. 将前端项目JiakaoFE（JiakaoBE中的【前端模板】内容）导入项目，然后打开index.html,修改访问后台的端口
    
    ```
    Commons.baseUrl = 'http://localhost:8888/'
    ```
    
    1. 点击右上角浏览器图标，则启动本地服务器。

### 实现数据字典类型
1. 将下载的layui项目导入，用于查看复制代码
2. 查看JiakaoFE下api下的init.json
    ...
    ```
3. 在page下新建目录`metadata/dictType`，并新建list.html
4. 将layui项目下的table.html内容copy到list.html,修改内容如下
    
    ```
    //修改url，cols
    table.render({
        elem: '#currentTableId',
        url: 'http://localhost:8888/dictTypes',
        toolbar: '#toolbarDemo',
