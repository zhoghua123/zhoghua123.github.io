...
#### propagation
1. 可以通过propagation属性设置事务的传播行为
    1. 用于指定当事务嵌套时，如何管理事务
    2. 当Service调用Service时，会出现事务嵌套
    3. 比如SkillServiceImpl层的test方法内部还调用了它自己的update方法，事务嵌套如下
        1. service调用test方法时会开启事务
        2. 内部调用update时也会开启事务，然后处理完成提交事务
        3. 提交test开启的事务
...
## 多个配置文件
1. 如果Spring配置文件的内容过多，可以采取多个配置文件的形式
2. 如果把所有的配置内容都放在applicationContext.xml中会越来越大
3. 可以将各个模块单独放一个配置文件：service、mybatis、事务等
4. 举例
    1. 有3个配置文件：
        ...
        ```
    2. 方法一：在人口配置文件分别倒入其他配置文件,外部使用不变
