...
1. 使用MyBatis实现dao层的几种方式
    1. 方法一:自定义dao实现类，在实现中调用SqlSession的相关方法(使用XML)
...
2. 常用注解
    1. `@Select、@Insert、@Update、@Delete、@Selectedkey`
    2. @Param：设置参数名，@Options：设置其他属性值，@CacheNamespace：对应`<cache>`
    3. @Results、@ResultMap：对应`<resultMap>`，@Result：对应`<id>`、`<result>`
    4. @One对应`<association>`，@Many对应`<collection>`
    5. @ConstructorArgs：对应`<constructor>`，@Arg：对应`<idArg>`、`<arg>`
    6. 可以使用`<script>`嵌套其他XML标签中的内容
    7. 参考资料：[https：//mybatis.org/mybatis-3/zh/java-api.html](https：//mybatis.org/mybatis-3/zh/java-api.html)
