...
15. **lldb（gdb）常用的控制台调试命令？**
    1. LLDB与GDB是什么?
        ...
        2. GDB: UNIX及UNIX-like下的调试工具。
        3. LLDB: 开源的内置于XCode的具有REPL(read-eval-print-loop)特征的Debugger，其可以安装C++或者Python插件。
        4. 两个都是调试用的Debugger，只是LLDB是比较高级的版本，或者在调试开发iOS应用时比较好用
        5. 就是打印窗口
    2. 如何使用? 
        1. 打局部断点,然后就进入调试器了.
    3. 一些Xcode调试快捷键：
