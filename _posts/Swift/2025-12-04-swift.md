...
### 闭包（Closure）重难点！！！
1. 网上有各种关于闭包的定义，觉得比较严谨的定义是：**一个函数和它所捕获的变量\常量环境组合起来，称为闭包**
    1. 一般指定义在函数内部的函数
...
## 属性
1. Swift中跟实例相关的属性可以分为2大类:存储属性（Stored Property）、计算属性（Computed Property）
...
#### 类型属性细节
1. 不同于存储实例属性，**必须**给存储类型属性设定初始值，因为类型没有像实例那样的init初始化器来初始化存储属性
