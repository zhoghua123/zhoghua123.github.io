...
## 结构体
1. 在 Swift 标准库中，绝大多数的公开类型都是结构体，而枚举和类只占很小一部分，比如Bool、Int、Double、 String、Array、Dictionary等常见类型都是结构体
...
## 类
### 类的初始化器
1. 类的定义和结构体类似，但编译器**并没有**为类自动生成可以传入成员值的初始化器
...
2. 类似于对文件进行copy、paste操作，产生了全新的文件副本。属于**深拷贝（deep copy）**
