---
layout: post
title: Swift第七章 协议、错误处理、局部作用域、泛型
category: Swift
tags: Swift
description: Swift
---
...
### 协议中的实例方法
1. 只有将协议中的**实例方法**标记为mutating，才允许结构体、枚举的具体实现修改自身内存
...
// 接收同时遵守Livable、Runnable协议、并且是Person或者其子类的实例。条件最多1个类类型Person限制
...
3. debugPrint调用的是CustomDebugStringConvertible协议的debugDescription
...
1. Self代表当前类型
    ...
    ```
2. Self一般用作返回值类型，**限定**返回值跟方法调用者必须是同一类型（也可以作为参数类型）
