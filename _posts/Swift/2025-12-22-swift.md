...
1. 元组类型的访问级别是所有成员类型最低的那个
    ...
    // (Dog, Person)整个元祖的访问级别是fileprivate，成员中取最小
...
1. 逃逸闭包不可以捕获inout参数
    ...
    ```
2. 原因分析： 如果逃逸闭包捕获了当前函数的inout参数，如果外部的参数是局部变量，早已销毁，就会造成空指针调用
