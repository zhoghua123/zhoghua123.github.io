...
9. 代码的初始化在`APP.js`中
    ...
    ```
    import React from 'react'; //导入node_modules文件夹下的React文件
    import {
      SafeAreaView,
      StyleSheet,
      ScrollView,
      ...
      Text,
      StatusBar,
    } from 'react-native';  //导入node_modules文件夹下的react-native文件内部的组件
  
    //程序的入口
    const App: () => React$Node = () => {   
      return (
          //根据iOS可以查看，这里就是keywindow的rootVC的View
