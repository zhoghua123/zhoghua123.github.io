...
*写这篇文章之前先要感谢一下@[敖老黑](http://www.jianshu.com/u/24e2b6d536e8)/@[_南山忆](http://www.jianshu.com/u/774b1d5616a7)这两位博主,谢谢他们的肩膀.*
### 写这篇文章的目的:
* 平时对手机硬件设备开发用的比较少
*  看其他人的博客自己觉得还是比较乱,尤其是在适配iOS>10.0,更是乱
*  AVFoundation框架比较大,功能太多,用到一点研究一点,慢慢深入

### 概念
AVFoundation的强大之处我在这里就不一一赘述了.我也看了有人翻译的苹果官方AVFoundation框架的全部中文文档,但是内容有点多,更重要的是有些类我们从来没用过,一下子学太多会吃不消,因此,还是一点点来吧.

##### 捕捉类:
就是AVFoundation框架中带Capture(捕捉)字样的所有类,这些类很强大,可以实现录视频/语音/拍照功能  
根据苹果官方文档常见的捕捉类的关系是这样的,如下图:
![苹果官方图片1.png](https://gitee.com/zhonghua123/blogimgs/raw/master/img/Foundation1.png)
下面就一一讲述一下这几个类:
...
**概念:**     
是AVFoundation 捕捉类(capture classes)的中心   
**分析:**     
capture:捕捉的意思   
...
* 可以添加输入(AVCaptureDeviceInput)和输出(有多种例如:AVCaptureMovieFileOutput)
* 通过[AVCaptureSession startRunning]启动从输入到输出的数据流，[AVCaptureSession stopRunning]停止流
* 属性sessionPreset:来定制输出的质量级别或比特率。

##### AVCaptureOutput
**概念:**     
是一个抽象类,决定了捕捉会话(AVCaptureSession)输出什么(一个视频文件还是静态图片)  
**作用:**     
...
![苹果官方图片2.png](https://gitee.com/zhonghua123/blogimgs/raw/master/img/Foundation2.png)
**概念:**     
...
AVCaptureOutput的一个抽象子类，用于将捕获的媒体静态图片(iOS10废弃掉了,使用AVCapturePhotoOutput)   
用户可以在实时的捕获资源下实时获取到高质量的静态图片,通过   captureStillImageAsynchronouslyFromConnection:completionHandler:方法(**也就是点击拍照动作方法**)     
 用户还可以配置静态图像输出以生成特定图像格式的静态图像。
* **AVCapturePhotoOutput(>ios10)**
**特点:**
  * 除了拥有AVCaptureStillImageOutput的功能外还支持更多格式
...
* **AVCapturePhotoOutput -capturePhotoWithSettings:delegate:方法说明:**     
该方法启动一个照片捕捉,也就是立即拍照的意思,通过下面的代理方法实现就能拿到实时图片数据:   
...
* 该方法不能重复使用settings,判定重复方法通过settings对像的uniqueID,然而该属性又是readonly,因此只能通过photoSettingsFromPhotoSettings:克隆了
* 初始化AVCaptureOutput对象,想给对象添加配置AVCapturePhotoSettings时,不能使用该方法,因为他就是拍照时使用的,而且他需要连接状况下才能使用,即捕捉会话先添加输入/输出后,正常连接,因为这个方法一旦调用就回立马书粗捕获到的数据,但此时刚刚初始化而已,还未添加到捕捉会话中,那么就使用 setPhotoSettingsForSceneMonitoring:方法初始化配置即可
* 遵守一大堆规则:  
 散光灯规则:必须支持该散光灯模式   
 即调用该方法前一定要判断是否支持闪光灯模式(如果不支持就回崩溃):  
...
FlashMode:AVCaptureDevice的属性
 ...
 注意:切换前必须先调用这个方法    lockForConfiguration:锁定设备,在进行切换
...
**概念:**     
 ...
 ![捕捉类关系图.png](https://gitee.com/zhonghua123/blogimgs/raw/master/img/Foundation3.png)
 
### 代码演示:    
上面讲了一堆概念,下面我们通过写一个模仿钉钉打卡拍照的demo来学会使用上面的捕捉类:(该demo适配iOS>10.0)    
界面分析如下: 
![Snip20171027_7.png](https://gitee.com/zhonghua123/blogimgs/raw/master/img/Foundation4.png)
> 原理:
...
@end
```
[github上Demo地址](https://github.com/zhoghua123/ZHWaterMarkCamera)
